...
export async function readStore(): Promise<ControlPlaneStore> {
  ...
  try {
    ...
    return parsed as ControlPlaneStore;
  } catch (e: any) {
    if (e?.code === "ENOENT") {
...
export async function upsertTaskStatus(taskId: number, status: TaskStatus) {
  const now = new Date().toISOString();
  const store = await readStore();
  store.tasks[String(taskId)] = { id: taskId, status, updatedAt: now };
  store.updatedAt = now;
  await writeStore(store);
  return store.tasks[String(taskId)];
}
